...
import { importTransactionsCsv, type CsvImportResult } from "./actions";
...
export function CsvImport() {
  ...
  const mutation = useMutation({
    mutationFn: (text: string) => importTransactionsCsv(text),
    onSuccess: (res) => {
      setResult(res);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Columns: <code>date</code>, <code>amount</code>, and optionally{" "}
            <code>type</code>, <code>category</code>, <code>description</code>.
          </p>
        </div>
        <input
          ref={inputRef}
