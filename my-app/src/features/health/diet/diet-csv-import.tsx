...
import { importDietCsv, type DietCsvImportResult } from "./actions";
import { Button } from "@/components/ui/button";

export function DietCsvImport() {
  const queryClient = useQueryClient();
  const inputRef = useRef<HTMLInputElement>(null);
  const [result, setResult] = useState<DietCsvImportResult | null>(null);

  const mutation = useMutation({
    mutationFn: (text: string) => importDietCsv(text),
    onSuccess: (res) => {
      setResult(res);
