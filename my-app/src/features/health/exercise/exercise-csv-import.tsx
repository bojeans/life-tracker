...
import { importExerciseCsv, type ExerciseCsvImportResult } from "./actions";
import { Button } from "@/components/ui/button";

export function ExerciseCsvImport() {
  const queryClient = useQueryClient();
  const inputRef = useRef<HTMLInputElement>(null);
  const [result, setResult] = useState<ExerciseCsvImportResult | null>(null);

  const mutation = useMutation({
    mutationFn: (text: string) => importExerciseCsv(text),
    onSuccess: (res) => {
      setResult(res);
